...
int main(int* argc, char** argv[])
{
	...
	MPI_Scatter(&package.front(), 4 * k, MPI_DOUBLE, &recieve1.front(), 4 * k, MPI_DOUBLE, 0,MPI_COMM_WORLD);                    //  Now requires only one scatter
	...
	MPI_Gather(&package2.front(), 8, MPI_DOUBLE, &recieve.front(), 8, MPI_DOUBLE, 0,MPI_COMM_WORLD);     //  Gather 8 values from each processor back to rank 0
	...
	MPI_Bcast(&u.front(), 2 * comm_sz - 2, MPI_DOUBLE, 0,MPI_COMM_WORLD);
	...
	MPI_Gather(&x.front(), k, MPI_DOUBLE, &x_all.front(), k, MPI_DOUBLE, 0,MPI_COMM_WORLD);
